...
pp   = PdfPages('outmy01plot.pdf')
tmp1 = plt.figure(1)
plot = open('outmy01plot-0.dat')
...
plt.yscale('symlog', linthreshy=1.55e+02)
...
plt.clf()
tmp4 = plt.figure(4)
plot = open('outmy01plot-5.dat')
plot = [line.split() for line in plot]
valx = [float(x[0]) for x in plot]
valy = [float(x[1]) for x in plot]
vale = [float(x[2]) for x in plot]
plt.hist( valx, vale, weights = valy, histtype='step', label=r'charged')
plt.yscale('symlog', linthreshy=5.00e-02)
plt.legend(frameon=False,loc='best')
