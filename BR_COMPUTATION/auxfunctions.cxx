...
// Added on 24.4.19, sonia.bouchiba@epfl.ch

Double_t get_VCKM(Quark_Type Din, Quark_Type Dout){
	// translate tyoe in index
	int i,j;
	...
	std::cout<<"VCKM01" << VCKM[0][1]<<std::endl;
	if(		Din== Quark_Type::down) 		j = 0;
	else if(Din== Quark_Type::strange)		j = 1;
	else if(Din== Quark_Type::bottom)		j = 2;
	
	if(		Dout== Quark_Type::up) 			i = 0;
	else if(Dout== Quark_Type::charm)		i = 1;
	else if(Dout== Quark_Type::top)			i = 2;
