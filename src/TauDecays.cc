...
bool TauDecays::decay(int idxOut1, Event& event) {
  ...
  if (sistersOut1.size() == 1) idxOut2Top = sistersOut1[0];
    ...
    // If more then one sister, select by preference mu+.
    int mup(-1);
    for (int i = 0; i < int(sistersOut1.size()); ++i) {
      int sn = out1.id() == -13 ? -1 : 1;
      int id = event[sistersOut1[i]].id();
      if      (id == sn * -13 && mup == -1) mup = sistersOut1[i];
  
    }
    if      (mup > 0) idxOut2Top = mup;
...
bool TauDecays::internalMechanism(Event &event) {
  ...
  // Produced from a D or B hadron decay with a single tau.
	...
	//cout << "HALLO SUSY 3 particles" << endl;
	//cout << "mediator.id(): " << mediator.id() << endl;
    ...
    // D or B meson decays into neutrino + tau.
      ...
      //cout << "HALLO SUSY 2 particles" << endl;
	  //cout << "mediator.id(): " << mediator.id() << endl;
    }
    hardME = hmeTwoFermions2W2TwoFermions.initChannel(particles);
...
void TauDecays::display(Event& event) {
	cout << "This is a display function of a TauDecay object" << endl;
	
	cout << "tau mode    : " << tauMode << endl;
	cout << "correlated  : " << correlated << endl;
	cout << "polarization: " << tauPol << endl;
	cout << "internalMode: " << internalMechanism(event) << endl;
	if(internalMechanism(event)==1) {cout << "mediator id: " << mediator.id() << endl;}
	else {cout << "internal 0, mediator id: " << mediator.id() << endl;}
