...
def print_accepted_events(n,count):

	if n / float(count) == int( n / float(count) ):
		print(str(n) + " events accepted!")
		
def get_candidate(pythia, muons, electrons):
	
	### look for one muon with pT > 12 GeV, and theta > 400 mrd
	mu_cands, e_cands = [], []
	
	for mu in muons:
		if mu.pT() > 12 and mu.theta() < 0.400:
#			print("muons pt: ", mu.pT())
			mu_cands.append(mu)
			
	for e in electrons:
		if e.theta() < 0.400 and e.pT() > 1:
#			print("electrons pt: ", e.pT())
			e_cands.append(e)
			
#	print(len(mu_cands), len(e_cands))
			
	if len(mu_cands) > 0 and len(e_cands) > 0:
		return (mu_cands, e_cands)
	else:
		return None
...
#	pythia.readString("PhaseSpace:pTHatMin = 20")
	...
	for b in branches_to_create:
		...
		tree.Branch( b, branches[b], f'{b}/D' )
