...
int main() {
	...
	if (!scanflag){
		...
		//tau0vec = {1., 2.}; 	// in ps, logarithmique
		//mNvec = {0.244,0.5};  // in GeV, linear
	}
	...
	*/
	...
	//for (int n(0); n<
			...
			if(cBflag){	// charged B, 521 and 541
				...
				if(!no3mesonflag) pythia.readFile("/home/bouchiba/PDM/pythia8240/cmndfiles/cB_wmeson_HNL"+HNL_id_s2+".cmnd"); // decay in nMeson, mu+ N
				else{
			...
			//if(!scanflag){
				...
				TFile *file;
				if (!scanflag) file = TFile::Open("./pytree_"+HNL_id_s+"_cB"+cBflag_s+"_"+no3mesonflag_s+"_"+tauMode_s+".root","recreate");
				//Event *event = &pythia.event;
				...
				for (int k(0); k<Bdaughters.size(); ++k){
					...
					Ndaughters.push_back({pythia.event[d1].daughter1(), pythia.event[d1].daughter2()});
						...
						if(!scanflag){
							...
							Particle& tauDecayer_ = pythia.event[d1];
							
							
							//cout << "internal process?: " << taudecayer_.internalMechanism(event);
