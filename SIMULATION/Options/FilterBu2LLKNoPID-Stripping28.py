...
#####################################################
    def _makeB2LLX( self, name, dilepton, hadrons, params, masscut = "(ADAMASS('B+')< 1500 *MeV" ):
        ...
        _matchCut = "mcMatch('[B_s0]CC') |mcMatch('[B+]CC') |mcMatch('[B0]CC') | mcMatch('[Lambda_b0]CC')"

        #for _Decay in _Decays:
        #    if _matchCut: _matchCut = "({0} || mcMatch({1}) )".format(_matchCut,_Decay)
        #    else:         _matchCut = "(mcMatch({1}) )".format(_matchCut,_Decay)
