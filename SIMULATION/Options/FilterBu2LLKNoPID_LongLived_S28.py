...
modified by Mick Mulder,     05-09-2017

tested with DaVinci v41r41p3
"""
...
class Bu2LLK_NoPID_LineBuilder(LineBuilder) :
    ...
    def __init__(self, name, config):
        ...
        MakeDiElectronsFromTracks.DiElectronMassMax = 5000.*MeV #just to give a high limit.  Not setting anything defaults it to 200 which is wrong.
        ...
        SelB2eeX = self._makeB2LLX(eeXLine_name,
                                   ...
                                   hadrons  = [ SelLambdasLL, SelLambdasDD, SelKshortsLL, SelKshortsDD ],
                                   params   = config,
                                   masscut  = "((ADAMASS('B+') <  %(BMassWindow)s *MeV) & (AM > 4500*MeV))" % config )
...
AllStreams = StrippingStream("Bu2KLL_NoPID_LongLived.Strip")
...
from DSTWriters.Configuration import SelDSTWriter, stripMicroDSTStreamConf, stripMicroDSTElements,stripDSTElements,stripDSTStreamConf
...
microDST=False# USED FOR TESTING

if microDST:
    SelDSTWriterElements = {
        'default'              : stripMicroDSTElements(pack=True,isMC=True)
        }
    SelDSTWriterConf = {
        'default'              : stripMicroDSTStreamConf(pack=True,isMC=True)
        }
else:
    SelDSTWriterElements = {
        'default'              : stripDSTElements(pack=True)
        }
    SelDSTWriterConf = {
        'default'              : stripDSTStreamConf(pack=True)
        }
...
stck = StrippingTCK(HDRLocation = '/Event/Strip/Phys/DecReports', TCK=0x41432800) 
